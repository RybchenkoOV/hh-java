 ...
 */
public class IntermediateStudentBuilder implements StudentBuilder {
  ...
  public Student build() {
    ...
    for (int i = 0; i < dataProvider.getSUBJECTS().length; i++) {
      allRanks.put(dataProvider.getSUBJECTS()[i], dataProvider.getIntermediateRanks()[i]);
    }
    Student intermediateStudent = new Student(name, group, course, allRanks);
    intermediateStudent.getStudentInfo(dataProvider.setFullInfo());
    return intermediateStudent;
  }
